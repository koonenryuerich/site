<?php
require 'library/phpmailer/class.phpmailer.php';
function spamcheck($field)
  {
	  //filter_var() sanitizes the e-mail
	  //address using FILTER_SANITIZE_EMAIL
	  $field=filter_var($field, FILTER_SANITIZE_EMAIL);
	
	  //filter_var() validates the e-mail
	  //address using FILTER_VALIDATE_EMAIL
	  if(filter_var($field, FILTER_VALIDATE_EMAIL))
	    {
	    	return TRUE;
	    }
	  else
	    {
	    	return FALSE;
	    }
  }

if (isset($_REQUEST['email']))
	{//if "email" is filled out, proceed
	
	//check if the email address is invalid
		$mailcheck = spamcheck($_REQUEST['email']);
		if ($mailcheck==FALSE)
		{
			echo "Invalid input";
		}
		else
		{//send email
			//$email = $_REQUEST['email'] ;
			$mail = new PHPMailer;
			$mail->IsSMTP();
			$mail->IsHtml(true);
			$mail->Host = 'smtp.gmail.com';
			$mail->Port       = 587;
			$mail->SMTPAuth = true;
			$mail->Username = 'kinkaidcommunityservice@gmail.com';
			$mail->Password = 'kinkaidcs2014';
			$mail->SMTPSecure = 'tls';
			
			$recipients = $_POST['email'];
			$bcc = $_POST['bcc'];
			$cc = $_POST['cc'];
			$subject = $_POST['subject'];
			$message = $_POST['message'];
			
			$recipientsarray = explode(" , ", $recipients);
			$bccarray = explode(" , ", $bcc);
			$ccarray = explode(" , ", $cc);
			
			
			$mail->From = 'kinkaidcommunityservice@gmail.com';
			$mail->FromName = 'Kinkaid Community Service Council';
			for ($i = 0;$i<count($recipientsarray);$i++){
				$mail->AddAddress($recipientsarray[$i]);
				
			}
			for ($i = 0;$i<count($bccarray);$i++){
				$mail->AddBCC($bccarray[$i]);
			}
			for ($i = 0;$i<count($ccarray);$i++){
				$mail->AddCC($ccarray[$i]);
			}
			
			//$mail->WordWrap = 50;
			
			$mail->Subject = $subject;
			$mail->Body = $message;
			
			if(!$mail->Send()) {
				echo 'Message could not be sent.';
				echo 'Mailer Error: ' . $mail->ErrorInfo;
				exit;
			}
			
			echo "Message has been sent. Please return to <a href = 'dashboard.php'>dashboard</a>";
			
			/*$headers = '';
			$headers .= "From: eric.yu@kinkaid.org"."\r\n";
			$targets = $_POST['email'];
			$subject = $_POST['subject'] ;
			$message = $_POST['message'] ;*/

		}
	}
	else
	{//if "email" is not filled out, display the form
		if (isset($_POST['emailvolunteers']) && isset($_POST['eventid'])){
			include 'header.php';
			$eventid = $_POST['eventid'];
			$query = "SELECT * FROM events where id='$eventid'";
			$result = queryMySql($query);
			$eventname = mysql_result($result, 0,'eventname');
			$query = "SELECT * FROM signups where eventid=$eventid";
			
			$result = queryMySql($query);
			
			$studentemails[] = array();
			$parentemails[] = array();
			for ($i = 0;$i<mysql_num_rows($result);$i++){
				$studentid = mysql_result($result, $i,'studentid');
				$query = "SELECT * FROM volunteers where id=$studentid";
				$studentresult = queryMySql($query);
				$studentemails[$i] = mysql_result($studentresult, 0,'email');
				$parentemails[$i] = mysql_result($studentresult, 0,'parentemail');
			}
			$studentemailstring = implode(" , ", $studentemails);
			$parentemailstring = implode(" , ", $parentemails);

			
			echo <<<END
			
			
			<div class="container">
			  <div class="row span10">
			    <div class = "margindiv">
				<h3 class = "muted">Email Form</h3>
				<form class = "form-horizontal" action = "emailvolunteers.php" method = "POST">
					<fieldset>
					<legend>Emailing Volunteers for Event: $eventname</legend>
					<p>You can either copy the emails into gmail, or use the simple messaging interface on the site. If you choose to email from the site, make sure there is a <b>space, comma, and space</b> after <b>every</b> email entry. E.G. 'eric.yu@kinkaid.org , someonelse@kinkaid.org'. Otherwise it will not work properly.</p>
					<br>
					<div class = " controls-row">
						<label style="display:inline; margin:20px;" class = "" >To:</label>
						<textarea onclick="this.focus();this.select()" name = 'email' rows = '5' cols = '40'>$studentemailstring</textarea>
						<label style="display:inline;margin:20px" class = "" >BCC:		</label>
						<textarea onclick="this.focus();this.select()" name = 'bcc' rows = '5' cols = '40'>$parentemailstring</textarea>
						<label onclick="this.focus();this.select()" style="display:inline;margin:20px" class = "" >CC:		</label>		
						<textarea name = 'cc' rows = '5' cols = '40'></textarea>
					</div>
					<br>
					<hr>
								
					<div style = margin:20px" class = "controls-row">
						<label style="display:inline; margin:20px;" class = "" >Subject:</label>
						<input name = 'subject' type = "text" class = "input-xlarge" id = "input01"/>
					</div>
					<br>
					<div style = margin:20px" class = "controls-row">
						<label style="display:inline; margin:20px;" class = "" >Message:</label>
						<br>
						<textarea class = "ckeditor" name = "message" cols "10" rows = "5" wrap ="type" ></textarea>
					</div>

					<div class = "form-actions">
						<button name = "edit" type = "submit" class = "btn btn-primary">Send Message</button>
					</div>
	
					</fieldset>
	
				</form>
				</div>
			</div>
			</div>
			
			<script type = "text/javascript" src="plugins/ckeditor/ckeditor.js"></script>
			
			
END;

	}
}


?>

      
      
      
</body>
</html>
